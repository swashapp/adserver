...
class AdsSendOneSwash implements ShouldQueue
{
    use Dispatchable;
    use Queueable;
    ...
    public function handle(AdsClient $adsClient): void
    {
        ...
        try {
            ...
            if (in_array($exception->getCode(), self::QUEUE_TRY_AGAIN_EXCEPTION_CODES, true)) {
                $logMessage = sprintf(
                    '[AdsSendOneSwash] Send command to (%s) with amount (%s) failed (message: %s).'
            ...
            Log::error(sprintf('[AdsSendOneSwash] Send command exception: %s', $exception->getMessage()));

            $logMessage = '[AdsSendOneSwash] Send command to (%s) with amount (%s) failed (message: %s).';
    ...
    {
        Log::error(sprintf('[AdsSendOneSwash] Job failed with exception (%s)', $exception->getMessage()));
        UserLedgerEntry::failAllRecordsInBatch($this->batchId, UserLedgerEntry::STATUS_NET_ERROR);
    }
}
